    ...
    @component('components.form-card', [
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <select name="id_user" id="id_user"
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ (old('id_user', $item->id_user) == $u->id) ? 'selected' : '' }} class="bg-slate-900">{{ $u->name }} ({{ $u->email }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="space-y-2">
                ...
                @if($item->foto_url)
                    <div class="mb-4 relative group rounded-2xl overflow-hidden border border-white/10 w-48 h-32">
                        <img src="{{ $item->foto_url }}" alt="Foto Kendaraan" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-all duration-500">
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent flex items-end p-3">
                            <span class="text-[8px] font-black text-white uppercase tracking-[0.2em] bg-emerald-500/80 px-2 py-1 rounded">Current Photo</span>
                        </div>
                    </div>
                @endif
                ...
                <p class="mt-2 text-[9px] text-slate-600 italic font-bold uppercase tracking-widest ml-1">Format: JPG, PNG, WEBP. Biarkan kosong jika tidak diubah.</p>
