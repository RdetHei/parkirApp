...
@section('content')
    @component('components.form-card', [
        ...
        'title' => 'Tambah Slot',
        'description' => 'Peta: ' . $parkingMap->name,
        ...
        'cardTitle' => 'Form Slot',
        'cardDescription' => 'Koordinat dalam pixel (sama dengan ukuran image overlay).',
        'action' => route('parking-maps.slots.store', $parkingMap),
        'method' => 'POST',
        'submitText' => 'Simpan'
    ])
        <div>
            <label for="code" class="block text-sm font-semibold text-gray-700 mb-2">Kode slot <span class="text-red-500">*</span></label>
            <input type="text" name="code" id="code" value="{{ old('code') }}" required placeholder="A1, B2, ..."
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="x" id="x" value="{{ old('x', 0) }}" required min="0" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('x') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="y" id="y" value="{{ old('y', 0) }}" required min="0" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('y') border-red-500 @enderror">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="width" id="width" value="{{ old('width', 80) }}" required min="1" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('width') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="height" id="height" value="{{ old('height', 40) }}" required min="1" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('height') border-red-500 @enderror">
        ...
        <div>
            ...
            <select name="area_parkir_id" id="area_parkir_id" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('area_parkir_id') border-red-500 @enderror">
                ...
                @foreach($areas as $a)
                    <option value="{{ $a->id_area }}" {{ old('area_parkir_id') == $a->id_area ? 'selected' : '' }}>{{ $a->nama_area }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-gray-500">Jika dipilih, status slot (kosong/terisi) mengikuti transaksi di area ini.</p>
        ...
        <div>
            ...
            <select name="camera_id" id="camera_id" class="block w-full px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('camera_id') border-red-500 @enderror">
                ...
                @foreach($cameras as $c)
                    <option value="{{ $c->id }}" {{ old('camera_id') == $c->id ? 'selected' : '' }}>{{ $c->nama }} ({{ $c->tipe }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="notes" id="notes" value="{{ old('notes') }}" placeholder="Contoh: Khusus mobil"
