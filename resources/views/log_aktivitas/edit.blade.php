...
@section('content')
    @component('components.form-card', [
        ...
        'cardIcon' => '<svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>',
            ...
            <div class="relative">
                ...
                <select name="id_user" id="id_user" required
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('id_user', $item->id_user) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->role }})</option>
                    @endforeach
                </select>
            </div>
        ...
        <div>
            ...
            <div class="relative">
                <textarea name="aktivitas" id="aktivitas" rows="4" required placeholder="Deskripsi aktivitas"
                          class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent @error('aktivitas') border-red-500 @enderror">{{ old('aktivitas', $item->aktivitas) }}</textarea>
            </div>
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="datetime-local" name="waktu_aktivitas" id="waktu_aktivitas" value="{{ old('waktu_aktivitas', $item->waktu_aktivitas?->format('Y-m-d\TH:i')) }}" required
