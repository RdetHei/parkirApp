...
<div class="max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold mb-4">Create Transaksi</h2>

    <form action="{{ route('transaksi.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select name="id_kendaraan" class="mt-1 block w-full border-gray-300 rounded-md">
                ...
                @foreach($kendaraans as $k)
                    <option value="{{ $k->id }}" {{ old('id_kendaraan') == $k->id ? 'selected' : '' }}>{{ $k->plat_nomor }} — {{ $k->jenis_kendaraan }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="waktu_masuk" value="{{ old('waktu_masuk') }}" class="mt-1 block w-full border-gray-300 rounded-md">
            @error('waktu_masuk')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="id_tarif" class="mt-1 block w-full border-gray-300 rounded-md">
                ...
                @foreach($tarifs as $t)
                    <option value="{{ $t->id }}" {{ old('id_tarif') == $t->id ? 'selected' : '' }}>{{ $t->jenis }} — {{ $t->harga }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="id_area" class="mt-1 block w-full border-gray-300 rounded-md">
                ...
                @foreach($areas as $a)
                    <option value="{{ $a->id }}" {{ old('id_area') == $a->id ? 'selected' : '' }}>{{ $a->nama_area ?? 'Area '.$a->id }}</option>
                @endforeach
            </select>
